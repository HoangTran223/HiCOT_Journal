...
class NeuroMax_Plus(NeuroMax):
    def __init__(self, weight_loss_CLT=1.0, weight_loss_CLC=1.0, weight_loss_DTC=1.0, weight_loss_TP=1.0,
                 ...
                 alpha_TP=20.0, sinkhorn_max_iter=500, contextual_embed_size=768, vocab=None, **kwargs):
        ...
        # This projection layer now uses contextual_embed_size
    ...
    def get_loss_TP(self, contextual_emb, indices):
        # contextual_emb: [batch_size, embed_dim] (batch local)
        # indices: [batch_size] (batch local, not global)
        minibatch_embeddings = contextual_emb  # Use the whole batch, do not index
    ...
    def get_loss_DTC(self, contextual_emb, theta, margin=0.2, k=5, metric='cosine'):
        # Uses contextual_emb directly
    ...
    def pairwise_euclidean_distance(self, x, y):
        # Ensure consistency with parent and other models
    ...
    def forward(self, indices, input, epoch_id=None):
        # Unpack inputs clearly
        bow = input[0].to(self.device)
        contextual_emb = input[1].to(self.device) # Used for ALL document-level losses
        ...
        # HiCOT contrastive losses using contextual_emb
        loss_TP = 0.0
        loss_CLC = 0.0
        loss_CLT = 0.0
        loss_DTC = 0.0
        ...
        # Total loss: NeuroMax original + HiCOT contrastive losses
